...
func PublicKeyInfoByMethod(certs []string, keyType x509.PublicKeyAlgorithm) (keynames, crts []string, publickeys []crypto.PublicKey, err error) {
	for _, cert := range certs {
		...
		case *rsa.PublicKey:
			ok = keyType == x509.RSA || keyType == x509.UnknownPublicKeyAlgorithm
		case ed25519.PublicKey:
...
func SignResponse(response *goxml.Xp, elementQuery string, md *goxml.Xp, signingMethod string, signFor int, signingKey uint8) (err error) {
	...
	names, certs, _, err := PublicKeyInfoByMethod(md.QueryMulti(nil, "md:IDPSSODescriptor"+SigningCertQuery), config.CryptoMethods[signingMethod].Type)
	if err != nil {
		return
	}
	privatekey, err := PrivateKeyByName(names[signingKey], "")
	if err != nil {
		return
	}

	cert := certs[signingKey]
...
func NewAuthnRequest(originalRequest, spMd, idpMd *goxml.Xp, virtualIDP string, idPList []string, acs string, wantRequesterID bool, spIndex, hubBirkIndex uint8) (request *goxml.Xp, sRequest SamlRequest, err error) {
	...
	if originalRequest != nil { // already checked for supported nameidformat
		...
		signingkey = config.KeySelectionMap[originalRequest.Query1(nil, "./@Destination")]
	}
...
func Map2saml(response *goxml.Xp, attrs map[string]interface{}) (err error) {
	...
	elems := []claimType{
		...
		{"acr", "./saml:Assertion/saml:AuthnStatement/saml:AuthnContext/saml:AuthnContextClassRef"},
	}
